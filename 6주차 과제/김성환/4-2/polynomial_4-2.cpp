...
void addTerm(char name, char* _c, char* _e) {
	...
	while (polys[index]->terms[curr] != NULL && polys[index]->nbr_terms != 0 && polys[index]->terms[curr]->expo > e) // 항의 갯수가 1개이상일때, 추가하는 항의 지수가 해당 항의 지수보다 작지 않을때까지 이동
	{
		curr++;
	}
	...
	polys[index]->terms[curr] = (term*)calloc(1, sizeof(term));
...
int createPoly(char name)
{
	...
	polys[n] = (polynomial*)calloc(1, sizeof(polynomial));
	polys[n++]->name = name;
	return 1;
}
// 다항식 name 생성
...
void diffPoly(char f1, char f2)
{
	...
	for (int i = 0; i < polys[index1]->nbr_terms; i++)
	{
		if (polys[index1]->terms[i]->expo == 0)
			break;
		polys[index2]->terms[i] = (term*)calloc(1, sizeof(term));
