...
//h파일에 프로토타입을 두는 함수들은 다른 소스 파일에서 호출하는 애들이고 c파일에 있는 프로토타입에 있는 함수는 c파일 내에서만
//호출되기 때문에 정의를 따로 해줌
...
SNode* find_snode(Artist* ptr_artist, char* title) {						//다른 부분에서도 쓰일거기 때문에 독립된 함수로 구성
...
Artist* add_artist(char* name) {
	...
	Artist* p = artist_directory[(unsigned char)name[0]];				//first node's address
	Artist* q = NULL;
...
void add_song(char* artist, char* title, char* path) {
	...
	//return NULL if not
	Artist *ptr_artist = find_artist(artist);					//검색을 해서 존재하면 artist 객체를 리턴해주도록 함.
	if (ptr_artist == NULL) {
...
void insert_to_index_directory(Song *ptr_song) { 
	...
	//insert add the snode into the single linked list at index_tavle[index]
...
void insert_node(Artist* ptr_artist, SNode* ptr_snode) {
	...
	// (4 case)  1.empty		2.at the front		3.at the end		4. in the middle
...
Artist* find_artist(char* name) {
	Artist* p = artist_directory[ (unsigned char)name[0] ];					//first artist with initial name[0](이름의 첫 글자)

	/*name [0] : 00000000 ~ 11111111 (1byte의 char)---->이를 정수로 받아들여 배열 인덱스로 사용할 수 있음*/
	/*1100111  -> (정수로 변환) -> 00000000000000000000000011001111 (x) */
	/*1100111  -> (정수로 변환) -> 11111111111111111111111111001111 (o)   -> 앞 숫자가 1인 경우, 정수로 변환할 때도 앞을 1로 채움*/
	/*이렇게 하면 1을 음수로 해석해주기때문에 name[0]앞에 unsigned char를 붙임*/
	while (p != NULL && strcmp(p->name, name) != 0) 
		p = p->next;
...
void save(FILE *fp) {
	for (int i = 0; i < NUM_CHARS; i++) {								//status code 
		Artist* p = artist_directory[i];
		while (p != NULL) {
			save_artist(p, fp);
			p = p->next;
...
void remove(int index) {
	...
	//find the sonde in the double linkged list of ptr_artist
